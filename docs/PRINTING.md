...
## 10. سرعة الطباعة — أين يذهب الوقت فعليًا

الشكوى المتكررة "الطباعة بطيئة" (~4-6 ثواني) ليست عطل واحد بل معمارية:
كل **هدف طباعة** (فاتورة كاشير مدمجة، أو تذكرة قسم بوضع "فوري") = دورة
رندر + إرسال كاملة لحالها، **بالتسلسل** — عن قصد، لأنه `render-server.js`
عنده طابور تنفيذ واحد (`queueRender`) بيعالج رندر واحد بكل مرة (Chrome/
Puppeteer ما بتتحمل رندرات متزامنة بشكل مستقر، انثبت هذا فعليًا بجلسات
تشخيص سابقة).

وضع **"فوري"** تحديدًا (تذكرة منفصلة لكل قسم، كلها على طابعة الكاشير —
مطلوب فعليًا لتتبع تحضير المطبخ، مش شي نلغيه) كان يضاعف المشكلة: طلب
بقسمين = دورتين كاملتين، **وكل دورة كانت تفتح اتصال طابعة منفصل** حتى
لو الوجهة نفسها بالضبط.

### التحسينات المطبّقة (2026-09-09)

1. **`--sleep=0.25`** بدل `--sleep=1` بـ`start-queue-worker.bat` — الـworker
   كان ينتظر لغاية ثانية كاملة قبل ما يفحص القائمة لما تكون فاضية. الآن
   ربع ثانية كحد أقصى، بدون كلفة تُذكر (استعلام DB خفيف على جدول صغير).
2. **اتصال طابعة واحد لكل دفعة طباعة "فوري"** بدل اتصال منفصل لكل قسم —
   `EscPosPrinterDriver::printMultipleReceiptImages()` (جديدة) بتفتح
   الاتصال مرة وحدة، وبتطبع كل صور الأقسام من خلاله (كل وحدة لسا بتتغذى
   وتنقص لحالها كإيصال فيزيائي منفصل)، وبتقفل مرة وحدة بالنهاية. بيوفر
   TCP handshake + تهيئة السائق المتكررة بلا داعي.

### ⚠️ هاي الملفات لازم تتزامن لكل محطة (نفس درس القسم 1)

`start-queue-worker.bat`, `app/Services/Printing/OrderPrintingService.php`,
`app/Services/Printing/PrinterService.php`,
`app/Services/Printing/Drivers/EscPosPrinterDriver.php`,
`app/Services/Printing/Contracts/PrinterDriverInterface.php` —
`verify-station.ps1` بيفحص وجود هالتحسينات كمان (قسم 8).

### ما لسا مش محسّن (بوعي، مش نسيان)

- **تسلسل الرندر نفسه** (`queueRender` بـrender-server.js) — عن قصد،
  تجربة سابقة أثبتت إنه رندرات Puppeteer متزامنة غير مستقرة. لو صار
  داعي حقيقي (مثلاً طلبات بـ3+ أقسام بشكل متكرر)، الحل الآمن الوحيد
  المقترح هو **صفحات Puppeteer متعددة من نفس المتصفح** (`browser.newPage()`
  لكل رندر بدل صفحة مشتركة واحدة) — مش رندرات متزامنة على نفس الصفحة،
  هاد يحتاج اختبار حذر قبل التطبيق، مش تغيير عشوائي.
- **زمن إرسال الصورة فعليًا للطابعة الحرارية** — محدود بسرعة الطابعة/
  الشبكة نفسها، خارج نطاق أي تحسين برمجي. الطباعة بالصور (لازمة لدعم
  العربي/RTL) أبطأ بطبيعتها من أوامر ESC/POS النصية على معظم الطابعات
  الحرارية.

## 11. تشخيص سريع — لائحة أوامر جاهزة
