...
بيفحص توقيعات الكود بالملفات المثبّتة فعليًا على الجهاز (مو الكود المركزي)،
وكمان (من 2026-09-10) إعدادات `.env` والاتصال الفعلي بقاعدة البيانات:

| الفحص | المطلوب | يدل على |
|---|---|---|
...
| `render-server.js` | `waitUntil: 'load'` بدون `networkidle0` | منع تأخير ~2 ثانية/رندر بلا داعي |
| `.env` | `POS_REGISTER_ID` موجودة وغير فاضية | المحطة معروفة الهوية لتوجيه الطباعة |
| `.env` | `POS_REGISTER_CODE` موجودة وغير فاضية | نفس الشي، بالاسم القابل للقراءة |
| اتصال DB فعلي | `php artisan migrate:status` بينجح | `DB_PASSWORD`/`DB_HOST` صحيحين فعليًا |

### حادثة POS-015 (2026-09-10) — ليش انضافت فحوص الـ`.env`

كل الفحوص فوق كانت **فحوص كود بس**. يوم 2026-09-10 اكتشفنا إنه POS-015
(PosRegister id=19) عندها `.env` افتراضي فاضي تمامًا — بدون `POS_REGISTER_ID`
ولا `POS_REGISTER_CODE`، و`DB_PASSWORD` غلط. الكود نفسه كان محدّث 100%،
بس التنصيب (`provision.ps1`) ما اكتمل صح على هالجهاز. النتيجة: الـ
`queue:work` كان يولّد اسم قائمة تالف (`pos-register-"=,default`) ويفشل
بالاتصال بقاعدة البيانات، فالطلبات كانت تتراكم بالطابور بدون ما حدا
يعالجها — والكاشير ما كان يعرف غير لما الورق ما طلع. فحص كود وحده ما
كان يكتشف هيك مشكلة أبدًا لأنه المشكلة كانت بالإعداد مش بالكود.
...
- ضد `installer\bundle\app` (قالب التنصيب، عبر `refresh-bundle.ps1`) — لازم
  `-SkipRuntimeChecks` هون، لأنه القالب طبيعي ما عنده `.env` أصلاً (بيتكتب
  فريش لكل جهاز وقت `provision.ps1`). بدون هالفلاغ، فحوص `.env`/قاعدة
  البيانات رح تفشل دايمًا على القالب بدون ما يكون في مشكلة حقيقية.

**لو فشل فحص كود**: انسخ الملف المحدّث من هاد الريبو (`o2-system-backend`)
...
**لو فشل فحص `.env`/اتصال قاعدة البيانات**: افتح `.env` بـNotepad
(مو PowerShell `Set-Content`/`Out-File` — بترمّز الملف UTF-16 وبتكسر
قراءة `start-queue-worker.bat` له)، وضيف/صحّح:
```
POS_REGISTER_ID="<رقم الـPosRegister بقاعدة البيانات، مش الرقم بالكود>"
POS_REGISTER_CODE="POS-0XX"
DB_PASSWORD="<نفس كلمة سر باقي المحطات>"
```
واحفظ، وبعدين أعد تشغيل `O2QueueWorker` زي فوق.
...
### حادثة POS-018 (2026-09-11) — إضافة Visual C++ Redistributable مفقودة

محطة جديدة (`cashier3`) طبعت "تمت الطباعة" بس بدون ورق فعلي. السبب:
إضافة `intl` بـPHP فشلت بالتحميل (`Unable to load dynamic library 'intl'`)
رغم إنه `php_intl.dll` وملفات ICU (`icudt71.dll` إلخ) كلها موجودة فعليًا
بحزمة PHP المرفقة — المشكلة كانت **Visual C++ Redistributable x64 غير
مثبّت على الجهاز نفسه** (احتياج حقيقي لأي DLL من PHP على ويندوز، ورسالة
الخطأ "specified module could not be found" مضلّلة، بتوحي إنه الملف نفسه
ناقص مش تبعية له). بدون `intl`، مكتبة الطباعة (`mike42/escpos-php`) بتنهار
فورًا بأي محاولة اتصال بطابعة (`IntlBreakIterator` غير موجود) —
`provision.ps1` صار الآن **يوقف التنصيب بالكامل** لو `intl` ما حمّلت،
بدل تحذير بسيط سهل يتفوت (راجع القسم بالأسفل عن فحص الإضافات).

**الحل**: تثبيت `https://aka.ms/vs/17/release/vc_redist.x64.exe` وإعادة
تشغيل الجهاز.
...
3. **`render-server.js`: شيل `networkidle0` من `page.setContent()`** —
   اكتُشف على POS-012 (2026-09-09) إنه كل رندر واحد كان ياخذ **~2 ثانية
   بالضبط**، وطلب 5 أقسام = ~8-10 ثواني كاملة قبل ما تبلش الطباعة أصلاً.
   قياس زمني دقيق لكل خطوة داخل `renderHtml()` (أضيف مؤقتًا، انشال بعدين)
   بيّن إنه كل الوقت كان بخطوة وحدة: `page.setContent(html, {waitUntil:
   ['load','networkidle0']})` — كل باقي الخطوات (فتح صفحة، انتظار خط،
   لقطة شاشة، إغلاق) كانت مجتمعة أقل من 100ms. السبب: `networkidle0`
   بينتظر عدم وجود اتصالات شبكة نشطة — وقوالب الإيصالات (`invoice.blade.php`,
   `department-ticket.blade.php`) HTML/CSS داخلي بالكامل، صفر مصادر خارجية
   (لا `@font-face`، لا `<img src=http>`، لا `<link>`) — يعني `networkidle0`
   عم ينتظر "هدوء شبكة" لصفحة ما كان فيها نشاط شبكة أصلاً من البداية. هاي
   مشكلة معروفة بـPuppeteer تحديدًا مع `setContent()` على محتوى بدون موارد
   خارجية. الحل: `waitUntil: 'load'` بس (انتظار الخط الصريح تحت هاد السطر
   ضل موجود ومسؤول عن أي تفاصيل خط، وقياسه أظهر 1-8ms فقط — يعني ما كان
   هو المشكلة إطلاقًا رغم إنه أول شي فكرنا فيه). النتيجة: ~2 ثانية/رندر
   رجعت لأقل من 100ms/رندر.
...
- **تسلسل الرندر نفسه** (`queueRender` بـrender-server.js) — عن قصد،
  تجربة سابقة أثبتت إنه رندرات Puppeteer متزامنة غير مستقرة. بعد إصلاح
  `networkidle0` (فوق) صار كل رندر <100ms، فحتى مع التسلسل الكامل طلب
  5 أقسام صار أقل من نص ثانية رندر إجمالي — الحاجة لصفحات متعددة صارت
  أضعف بكتير، بس لسا نفس المبدأ لو ظهرت حاجة فعلية لاحقًا: **صفحات
  Puppeteer متعددة من نفس المتصفح** (`browser.newPage()` لكل رندر بدل
  صفحة مشتركة واحدة) — مش رندرات متزامنة على نفس الصفحة، هاد يحتاج
  اختبار حذر قبل التطبيق، مش تغيير عشوائي.
