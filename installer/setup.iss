...
begin
  ...
  // ما منعبّي قيمة افتراضية هون عمداً (كانت XP-80C) - أكتر من مرة صار
  // تركيب فعلي بالاسم الافتراضي دون قصد لأنه الحقل كان معبّى مسبقاً وسهل
  // المرور عليه بدون تغييره، رغم إنه NextButtonClick تحت بيرفض المتابعة
  // لو الحقل فاضي - فراغه إجباري يجبر كل تركيب يكتب اسم الطابعة الحقيقي
  // المطابق تماماً لما هو مسجّل بويندوز (Devices and Printers) بنفسه.

  SvcPage := CreateInputQueryPage(DbPage.ID,
    'حساب تشغيل الخدمات وتفعيل المحطة',
    'حساب الويندوز، وكود تفعيل نقطة البيع (إجباري)',
    ...
    'كود التفعيل: إجباري (من 2026-09-10) - اطلبه من الأدمن قبل ما تبلش ' +
    'التنصيب (صالح 15 دقيقة بس). بيربط هالجهاز بمحطة كاشير محددة (PosRegister) ' +
    'فما تختلط طلبات الطباعة بين المحطات - محطة بدون هالربط طباعتها معطوبة ' +
    'تماماً، مش بس بدون عزل (راجع حادثة POS-015، 2026-09-10).');
...
begin
  ...
  begin
    ...
    if SvcPage.Values[2] = '' then
    begin
      MsgBox('لازم تدخل كود تفعيل نقطة البيع - إجباري الآن، اطلبه من الأدمن قبل المتابعة. بدونه طباعة هالمحطة رح تكون معطوبة تماماً.', mbError, MB_OK);
      Result := False;
    end;
  end;
end;
